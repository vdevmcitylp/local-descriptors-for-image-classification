	...
	for img in tqdm(images):
		...
		for x in range(1, img_height + 1):
			for y in range(1, img_width + 1):
				...
				s1 = heaviside((img[x-1, y-1]-img[x+1, y+1]+img[x, y])+(img[x-1, y-1]-img[x, y])*(img[x+1, y+1]-img[x, y]))

				s2 = heaviside((img[x-1, y]-img[x+1, y]+img[x, y])+(img[x-1, y]-img[x, y])*(img[x+1, y]-img[x, y]))*2

				s3 = heaviside((img[x-1, y+1]-img[x+1, y-1]+img[x, y])+((img[x-1, y+1]-img[x, y])*(img[x+1, y-1]-img[x, y])))*4

				s4 = heaviside((img[x, y+1]-img[x, y-1]+img[x, y])+((img[x, y+1]-img[x, y])*(img[x, y-1]-img[x, y])))*8
