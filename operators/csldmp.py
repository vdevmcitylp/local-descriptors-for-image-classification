	...
	for img in tqdm(images):
		...
		for x in range(1, img_height + 1):
			for y in range(1, img_width + 1):
				...
				s1 = sigmoid(-(img[x-1, y-1] - img[x, y]) * (img[x, y] - img[x+1, y+1]))
				s2 = sigmoid(-(img[x-1, y] - img[x, y]) * (img[x, y] - img[x+1, y]))*2
				s3 = sigmoid(-(img[x-1, y+1] - img[x, y]) * (img[x, y] - img[x+1, y-1]))*4
				s4 = sigmoid(-(img[x, y+1] - img[x, y]) * (img[x, y] - img[x, y-1]))*8
