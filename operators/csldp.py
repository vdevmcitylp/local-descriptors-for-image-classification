""" Center Symmetric Local Derivate Pattern """
	...
	for img in tqdm(images):
		...
		for x in range(1, img_height + 1):
			for y in range(1, img_width + 1):
				...
				s1 = heavisideLDP((img[x-1, y-1]-img[x, y])*(img[x, y]-img[x+1, y+1]))

				s2 = heavisideLDP((img[x-1, y]-img[x, y])*(img[x, y]-img[x+1, y]))*2

				s3 = heavisideLDP((img[x-1, y+1]-img[x, y])*(img[x, y]-img[x+1, y-1]))*4

				s4 = heavisideLDP((img[x, y+1]-img[x, y])*(img[x, y]-img[x, y-1]))*8
