...
def csLTP():
	...
	zeroHorizontal = np.zeros(72).reshape(2, 36)
	zeroVertical = np.zeros(64).reshape(32, 2)
	...
	for img in grayscaleImgList:
		...
		for x in range(2, 33):
			for y in range(2, 33):
				...
				#s2 = sigmoid(img[x-1, y] - img[x+1, y])*3
				s3 = sigmoid(img[x-2, y+2] - img[x+2, y-2])*3
				#s4 = sigmoid(img[x, y+1] - img[x, y-1])*27
